...
3. **左クリックまたは右クリック**します。
   - **左クリック**: 通常の要素取得（要素が実行される場合があります）
   - **右クリック**: 要素を実行せずに情報取得（ダイアログボタンなどに最適）
4. コンソールに要素の情報と生成されたRPAパスが表示されます。
5. 続けて他の要素をクリックして記録できます。
6. 終了するには `ESC` キーを押します。

> [!TIP]
> **ダイアログボタンの取得**: エラーダイアログの「OK」ボタンなど、クリックすると消えてしまう要素は**右クリック**で取得してください。右クリックではボタンが押されないため、ダイアログが閉じずに情報を取得できます。
...
3.  `Click the element for 'エイリアス名'` と表示されたら、対象のUI要素を**左クリックまたは右クリック**します。
    - **右クリック推奨**: ダイアログボタンなど、クリックすると消える要素の場合
...
- `--legacy`: レガシーモードを有効にします。Win32 API (`SetFocus`) を優先的に使用してフォーカス制御を行います。レガシーアプリでフォーカスが正しく当たらない場合に使用してください。
...
> [!TIP]
> **レガシーモードの活用**: 標準のフォーカス制御（UI Automation経由）で意図しない要素にフォーカスが飛んでしまう場合は、`--legacy` オプションを試してください。これにより、より低レベルな Win32 API を使用してフォーカスを設定します。
